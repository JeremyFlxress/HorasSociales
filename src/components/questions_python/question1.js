...
const QuizPage = () => {
  ...
  const [shownQuestions, setShownQuestions] = useState(new Set());

  useEffect(() => {
    const shuffled = [];
    const questionSet = new Set();

    while (shuffled.length < 35 && questionSet.size < questions.length) {
      const randomIndex = Math.floor(Math.random() * questions.length);
      if (!shownQuestions.has(randomIndex)) {
    ...
    setShownQuestions(new Set([...shownQuestions, ...questionSet]));
  }, []);
